...
import { useSidebar } from '@/providers/sidebar-provider/sidebar-hook';
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      variant='outline'
      size={'sm'}
      className={cn('text-foreground border-input text-xs', className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
    ...
    >
      {isMobile ? (
        ...
        <>
          <ChevronsLeft className={cn(!open && 'transition-rotate rotate-180 duration-200 ease-linear')} />
          {open && <span className='text-xs'>Свернуть меню</span>}
        </>
      )}
    </Button>
  );
...
function SidebarInput({ className, ...props }: React.ComponentProps<typeof BaseInput>) {
  return (
    <BaseInput
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  return (
    <ul
      ...
      className={cn(
        'border-input ms-3.5 flex min-w-0 translate-x-px flex-col gap-0.5 border-l px-2.5 py-0.5',
        'group-data-[collapsible=icon]:hidden',
        className
      )}
      {...props}
...
function SidebarMenuSubButton({
  ...
  return useRender({
    ...
    props: {
      ...
      'data-active': isActive,
      className: cn(
        'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground outline-hidden flex h-fit min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 py-1 focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>svg]:size-4 [&>svg]:shrink-0',
